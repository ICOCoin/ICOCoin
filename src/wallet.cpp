...
// ICOCoin: optional setting to unlock wallet for block minting only;
...
// ICOCoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // ICOCoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // ICOCoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // ICOCoin: not avatar mode
                        {
...
// ICOCoin: create coin stake transaction
...
// ICOCoin: check 'spent' consistency between wallet and txindex
// ICOCoin: fix wallet spent state according to txindex
...
// ICOCoin: disable transaction (only for coinstake)
